...
namespace API.Controladores.Accesorios
{
    ...
    [ApiController]
    public class ProveedoresAccesoriosController : ControllerBase
    {
        private readonly ModuloAccesoriosExplosivosContext context;
        private readonly IMapper mapper;

        public ProveedoresAccesoriosController(ModuloAccesoriosExplosivosContext context, IMapper mapper)
        {
        ...
        {
            var listaModelosDB = await context.ProveedoresAccesorios
                .Where(d => !d.Borrado)
                .ToListAsync();

            var respuestaValidada = RespuestaGenericaDTO<List<ProveedoresAccesorios>>.CrearRespuestaExitosa(listaModelosDB);
        ...
        {
            var modeloDB = await context.ProveedoresAccesorios
                .FirstOrDefaultAsync(x => x.Id == id);

            var respuestaValidada = RespuestaGenericaDTO<ProveedoresAccesorios>.CrearRespuestaExitosa(modeloDB);
        ...
        [AllowAnonymous]
        [HttpPost("[action]")]
        public async Task<ActionResult> Registrar([FromBody] RegistrarProveedoresAccesorios modelo)
        {
            ...
            var nuevoRegistro = mapper.Map<ProveedoresAccesorios>(modelo);

            nuevoRegistro.IdUnico = Guid.NewGuid().ToString();

            await context.ProveedoresAccesorios.AddAsync(nuevoRegistro);
        ...
        [AllowAnonymous]
        [HttpPut("[action]")]
        public async Task<ActionResult> Modificar([FromBody] ModificarProveedoresAccesorios modelo)
        {
            if (modelo.Id <= 0)
            {
            ...
            var modeloAValidar = mapper.Map<RegistrarProveedoresAccesorios>(modelo);
            ...
            var existente = await context.ProveedoresAccesorios
        ...
        {
            var modeloDB = await context.ProveedoresAccesorios.FirstOrDefaultAsync(x => x.Id == id);
        ...
        private async Task<(bool, string)> ComprobarModeloAsync(RegistrarProveedoresAccesorios modelo, int? idModelo = null)
        {
            ...
            {
                existe = await context.ProveedoresAccesorios
