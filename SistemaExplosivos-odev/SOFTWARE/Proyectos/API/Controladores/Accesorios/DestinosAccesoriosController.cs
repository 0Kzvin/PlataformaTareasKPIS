...
namespace API.Controladores.Accesorios
{
    ...
    [ApiController]
    public class DestinosAccesoriosController : ControllerBase
    {
        private readonly ModuloAccesoriosExplosivosContext context;
        private readonly IMapper mapper;

        public DestinosAccesoriosController(ModuloAccesoriosExplosivosContext context, IMapper mapper)
        {
        ...
        {
            var listaModelosDB = await context.DestinosAccesorios
                .Where(d => !d.Borrado)
                .ToListAsync();

            var respuestaValidada = RespuestaGenericaDTO<List<DestinosAccesorios>>.CrearRespuestaExitosa(listaModelosDB);
        ...
        {
            var modeloDB = await context.DestinosAccesorios
                .FirstOrDefaultAsync(x => x.Id == id);

            var respuestaValidada = RespuestaGenericaDTO<DestinosAccesorios>.CrearRespuestaExitosa(modeloDB);
        ...
        [AllowAnonymous]
        [HttpPost("[action]")]
        public async Task<ActionResult> Registrar([FromBody] RegistrarDestinosAccesorios modelo)
        {
            ...
            var nuevoRegistro = mapper.Map<DestinosAccesorios>(modelo);

            nuevoRegistro.IdUnico = Guid.NewGuid().ToString();

            await context.DestinosAccesorios.AddAsync(nuevoRegistro);
        ...
        [AllowAnonymous]
        [HttpPut("[action]")]
        public async Task<ActionResult> Modificar([FromBody] ModificarDestinosAccesorios modelo)
        {
            if (modelo.Id <= 0)
            {
                var respuesta = RespuestaGenericaDTO<string>.CrearRespuestaConErrores("El Id del destino es inválido");

                return BadRequest(respuesta);
            }

            var modeloAValidar = mapper.Map<RegistrarDestinosAccesorios>(modelo);
            ...
            var existente = await context.DestinosAccesorios
                .FirstOrDefaultAsync(x => x.Id == modelo.Id && !x.Borrado);

            if (existente == null)
            {
                var respuesta = RespuestaGenericaDTO<string>.CrearRespuestaConErrores("No se encontró un destino con el Id proporcionado");
                return BadRequest(respuesta);
            }
        ...
        {
            var modeloDB = await context.DestinosAccesorios.FirstOrDefaultAsync(x => x.Id == id);
        ...
        private async Task<(bool, string)> ComprobarModeloAsync(RegistrarDestinosAccesorios modelo, int? idModelo = null)
        {
            if (string.IsNullOrEmpty(modelo.Nombre))
            {
                return (false, "El nombre del destino es obligatorio");
            }
            ...
            {
                existe = await context.DestinosAccesorios
            ...
            if (existe)
                return (false, "Ya existe un destino con ese mismo nombre");
