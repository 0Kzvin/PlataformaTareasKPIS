...
namespace API.Controladores.Gerencia
{
    ...
    [ApiController]
    public class EstacionesGerenciaController : ControllerBase
    {
        private readonly ModuloGerenciaExplosivosContext context;
        private readonly IMapper mapper;

        public EstacionesGerenciaController(ModuloGerenciaExplosivosContext context, IMapper mapper)
        {
        ...
        {
            var listaModelosDB = await context.Estaciones
                .Where(d => !d.Borrado)
                .ToListAsync();

            var respuestaValidada = RespuestaGenericaDTO<List<EstacionesGerencia>>.CrearRespuestaExitosa(listaModelosDB);
        ...
        {
            var modeloDB = await context.Estaciones
                .FirstOrDefaultAsync(x => x.Id == id);

            var respuestaValidada = RespuestaGenericaDTO<EstacionesGerencia>.CrearRespuestaExitosa(modeloDB);
        ...
        [AllowAnonymous]
        [HttpPost("[action]")]
        public async Task<ActionResult> Registrar([FromBody] RegistrarEstacionesGerencia modelo)
        {
            ...
            var nuevoRegistro = mapper.Map<EstacionesGerencia>(modelo);

            nuevoRegistro.IdUnico = Guid.NewGuid().ToString();

            await context.Estaciones.AddAsync(nuevoRegistro);
        ...
        [AllowAnonymous]
        [HttpPut("[action]")]
        public async Task<ActionResult> Modificar([FromBody] ModificarEstacionesGerencia modelo)
        {
            if (modelo.Id <= 0)
            {
                var respuesta = RespuestaGenericaDTO<string>.CrearRespuestaConErrores("El Id de la estación es inválido");

                return BadRequest(respuesta);
            }

            var modeloAValidar = mapper.Map<RegistrarEstacionesGerencia>(modelo);
            ...
            var existente = await context.Estaciones
                .FirstOrDefaultAsync(x => x.Id == modelo.Id && !x.Borrado);

            if (existente == null)
            {
                var respuesta = RespuestaGenericaDTO<string>.CrearRespuestaConErrores("No se encontró una estación con el Id proporcionado");
                return BadRequest(respuesta);
            }
        ...
        {
            var modeloDB = await context.Estaciones.FirstOrDefaultAsync(x => x.Id == id);
        ...
        private async Task<(bool, string)> ComprobarModeloAsync(RegistrarEstacionesGerencia modelo, int? idModelo = null)
        {
            if (string.IsNullOrEmpty(modelo.Nombre))
            {
                return (false, "El nombre de la estación es obligatorio");
            }
            ...
            {
                existe = await context.Estaciones
            ...
            if (existe)
                return (false, "Ya existe una estación con ese mismo nombre");
