...
const onSubmit = async () => {
  ...
  try {
    ...
    // Router Logic - SistemaProductividad
    // Determine route based on Roles (Mock logic until granular role data is perfect)
    const roles = resp.payload.payload.datosUsuario.roles || [];
    const esSuperAdmin = roles.some(r => r === 'SuperAdmin' || r === 'Administrador');
    
    if (esSuperAdmin) {
      await router.push({ name: 'DashboardGlobal' });
    } else {
      await router.push({ name: 'DashboardDepartamento' });
    }
...
// -- Helpers de Ruteo --
// (Deleted unused legacy routing logic)
</script>
